...
##### MODIFICACIONES RICCI:
# Ordenamiento natural de la lista de frames
...
#                command_input['output_name'] = command_input['traj_file'].split('.')[0]
        ...
        print(command_input['traj_file'], type(command_input['traj_file']))
        
        if command_input['traj_file'] == []: # CORREGIDO
            print("You need to specify .npy files to be read")
            sys.exit(1)

#### AGREGADO POR RICCI
        command_input['traj_file'].sort(key=self.natural_keys) # Ordenamiento natural
        print("Lista de frames ordenada: ", command_input['traj_file'])
...
#### AGREGADO POR RICCI ####
    # Natural sorting: http://nedbatchelder.com/blog/200712/human_sorting.html
    def atoint(self, text): # Devuelve un int si el string es un digito
        key = int(text) if text.isdigit() else text
        return key

    def natural_keys(self, text): # Define la clave de ordenamiento basado en los digitos
        clave =  [ self.atoint(c) for c in re.split( r'(\d+)', text )] # Split y llamada a atoint
        return clave
