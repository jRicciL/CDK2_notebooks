...
#import fnmatch
import pylab
# Necesario instalar: pip install peel
...
#        scipy.cluster.hierarchy.dendrogram(result)
    ...
    def separate_cluster_traj(self,cluster_result,number_clusters,file_name,traj_file,output_file):
        ...
        for i in range(number_clusters):
            list_writes[i] = open('cluster_'+ str(i)+'.pdb','w')
...
#            print "Cluster_res for {0} is {1}".format(i, cluster_results[i])
    ...
    def find_centroids(self, list_of_clusters, outputPrefix):
        ...
        if (self.indexToFrame == {}) and (self.indexToNpyFile != {}):
            ...
            with open('%scluster_reps.csv' %(outputPrefix),'w') as of:
                of.write('\n'.join([str(i) for i in centroid_list]))
            with open('%scluster_members.csv' %(outputPrefix),'w') as of:
                for cluster in list_of_clusters:
                ...
                for line in fo:
                    ...
                    if 'END' == line[:3]:
                        if frameData != '':
                            ...
                            with open(thisOutputFileName,'w') as of:
                                of.write(frameData)
                        frameData = ''
                        frameCounter += 1
                if frameData != '':
...
'''
        ...
        for clusterIndex, clusterMap in enumerate(clusterMaps):
            ...
            with open('%s/visualize.vmd' %(outputDir),'w') as of:
        ...
        with open('%svisualizeAll.vmd' %(outputPrefix),'w') as of:
            of.write(plotAll)
