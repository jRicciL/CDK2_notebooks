...
#*** MODIFICADO POR RICCI:
#*** Modificado de python2 a python3
#*** Se agraga una validación en la línea 47, que permite ejecutar el script sin tener en directorios distintos
#*** los *frame_\d*.npy y los archivos de "color"
...
#*** EJECUCIÓN:
#   python3 binding_site_overlap.py -f *frame_*npy  -c --csv
...
#****** AGREGADOS
        self.hydrophilic_coordinates = []
        self.hydrophobic_coordinates = []
...
#****** AGREGADOS los analisis para hydrophilic, hydrophobic
        self.hydrophilic_filename = []
        self.hydrophobic_filename = []
        

    #traj_file is a list containing all the filenames of npy files found
#### Ya NO ES NECESARIO: #Must run colored vs uncolored from different directories else uncolored will be reading too many files
    ...
    def read_traj(self,traj_file, color):
        ...
        expr = re.compile('frame_([0-9]+).npy')
        #frameNo = re.findall(traj_file, 'frame_([0-9]+).npy')
        #frameNo = re.findall(traj_file, 'frame_([0-9]+)_aromatic.npy')
### La lista sortedFramesAndNames se crea solo con los archivos volumetricos, omitiendo los de color
### Por lo tanto ambos tipos de archivos deben estar en el mismo directorio
        sortedFramesAndNames = [(int(re.findall(expr,name)[0]), name) for name in traj_file if re.findall(expr, name)]
...
#****** AGREGADOS
                hydrophilic_file = self.volumetric_filename[i].strip('.npy')+'_hydrophilic.npy'
                hydrophobic_file = self.volumetric_filename[i].strip('.npy')+'_hydrophobic.npy'
...
#****** AGREGADOS
                self.hydrophilic_filename.append(hydrophilic_file)
                self.hydrophilic_coordinates.append(set([tuple(dummy_atom[:3]) for dummy_atom in numpy.load(hydrophilic_file) if dummy_atom[3] > 0.02]))
                self.hydrophobic_filename.append(hydrophobic_file)
                self.hydrophobic_coordinates.append(set([tuple(dummy_atom[:3]) for dummy_atom in numpy.load(hydrophobic_file) if dummy_atom[3] > 0.02]))
...
class main():
    ...
    def __init__(self,argv):
        ...
        parser.add_option("-f", dest = "filename", action = "callback", callback = vararg_callback, 
            help = "All files from POVME that you want to run similarity calculations on.")
        ...
        with open('indexMapToFrames.csv','w') as csvfile:
...
### AGREGADO
        hydrophilic_overlap = Overlap(file_input.hydrophilic_coordinates)
        hydrophobic_overlap = Overlap(file_input.hydrophobic_coordinates)
...
#### AGREGADO POR RICCI 
            hydrophilic_num_overlap = numpy.empty([file_input.frames, file_input.frames], dtype = float) # Matriz vacia de nxn frames
            for f1 in range(num_frames):
                for f2 in range(num_frames):
                    hydrophilic_num_overlap[f1, f2] = hydrophilic_overlap.number_overlap(f1, f2) # numero de puntos solapados para el par de frames

            hydrophobic_num_overlap = numpy.empty([file_input.frames, file_input.frames], dtype = float)
            for f1 in range(num_frames):
                for f2 in range(num_frames):
                    hydrophobic_num_overlap[f1, f2] = hydrophobic_overlap.number_overlap(f1, f2)
####                    
...
#### 
            hydrophilic_tanimoto_matrix = numpy.empty([file_input.frames,file_input.frames],dtype=float)
            hydrophobic_tanimoto_matrix = numpy.empty([file_input.frames,file_input.frames],dtype=float)            
#### 
...
#### 
                    hydrophilic_result = hydrophilic_overlap.tanimoto_overlap(hydrophilic_num_overlap[f1,f2],f1,f2)
                    hydrophilic_tanimoto_matrix[f1,f2] = hydrophilic_result
                    hydrophilic_tanimoto_matrix[f2,f1] = hydrophilic_result

                    hydrophobic_result = hydrophobic_overlap.tanimoto_overlap(hydrophobic_num_overlap[f1,f2],f1,f2)
                    hydrophobic_tanimoto_matrix[f1,f2] = hydrophobic_result
                    hydrophobic_tanimoto_matrix[f2,f1] = hydrophobic_result
#### Modificado para que el promedio tome en cuenta los colores hydrophilic e hydrophobic
                    average_similarity = (hbondAcc_result #+ hbondDon_result + aromatic_result
                        + tanimoto_result+
                        hydrophilic_result + hydrophobic_result )/4
...
####
            print("JR added: Hydrophilic")
            print(hydrophilic_tanimoto_matrix)
            print("JR added: hydrophobic")
            print(hydrophobic_tanimoto_matrix)
...
#### Se guardan los resultados extra
            numpy.savetxt(os.getcwd()+'/POVME_Tanimoto_matrix_hydrophilic.csv', hydrophilic_tanimoto_matrix, delimiter=',')
            numpy.savetxt(os.getcwd()+'/POVME_Tanimoto_matrix_hydrophobic.csv', hydrophobic_tanimoto_matrix, delimiter=',')
...
#### Se guardan los resultados extra
            numpy.save('hydrophobic_tanimoto_matrix.npy', hydrophobic_tanimoto_matrix)
            numpy.save('hydrophobic_tanimoto_matrix.npy', hydrophobic_tanimoto_matrix)
...
#### 
            hydrophilic_tversky_matrix =  numpy.empty([file_input.frames,file_input.frames],dtype=float)
            hydrophobic_tversky_matrix = numpy.empty([file_input.frames,file_input.frames],dtype=float)            
#### 
...
####
                    hydrophilic_tversky_matrix[f1,f2] = hydrophilic_overlap.tversky_overlap(hydrophilic_num_overlap[f1,f2],f1,f2)
                    hydrophobic_tversky_matrix[f1,f2] = hydrophobic_overlap.tversky_overlap(hydrophobic_num_overlap[f1,f2],f1,f2)
#### Modificado para que el promedio tome en cuenta los colores hydrophilic e hydrophobic
                    colored_tversky_matrix[f1,f2] = (hbondAcc_tversky_matrix[f1,f2] + # hbondDon_tversky_matrix[f1,f2] + aromatic_tversky_matrix[f1,f2] +
                        tversky_matrix[f1,f2] + 
                        hydrophilic_tversky_matrix[f1,f2] + hydrophobic_tversky_matrix[f1,f2])/4
        ...
        if (options.color):
            ...
            ####
            print("JR added: Hydrophilic")
            print(hydrophilic_tversky_matrix)
            print("JR added: hydrophobic")
            print(hydrophobic_tversky_matrix)
...
#### Se gusardan los resultados extra
            numpy.savetxt(os.getcwd()+'/POVME_Tversky_matrix_hydrophilic.csv', hydrophilic_tversky_matrix, delimiter=',')
            numpy.savetxt(os.getcwd()+'/POVME_Tversky_matrix_hydrophobic.csv', hydrophobic_tversky_matrix, delimiter=',')
...
#### Se gusardan los resultados extra
            numpy.save('hydrophobic_tversky_matrix.npy', hydrophobic_tversky_matrix)
            numpy.save('hydrophobic_tversky_matrix.npy', hydrophobic_tversky_matrix)
